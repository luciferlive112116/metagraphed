...
import { ANALYTICS_WINDOWS } from "../workers/config.mjs";
...
describe("graphql — chain_registrations (#5876, Postgres-tier + D1-live fallback)", () => {
  ...
  test("resolves the D1-live tier: per-subnet leaderboard + network rollup", async () => {
    ...
    // the per-subnet registrants (4) -- a hotkey registering on several subnets
    // counts once.
    assert.deepEqual(got.network, {
      distinct_registrants: 3,
      registrations: 12,
  ...
  test("every window analyticsWindow accepts is accepted here too", async () => {
    for (const window of Object.keys(ANALYTICS_WINDOWS)) {
      const { status, body } = await gql(
